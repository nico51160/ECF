...
class Voiture
{
        ...
        if ($pdo->execute()) {
            ...
            foreach ($_FILES['images']['type'] as $key => $type) {
                if (!in_array($type, ['image/png', 'image/jpeg'])) {
                    return 'error: le type du fichier est non autorisé'; 
                }
            }
    ...
    {
        $pdo = Cnx::Connect()->prepare("DELETE FROM voiture_images where voiture_id=:id");
        $pdo->bindParam(':id', $id, PDO::PARAM_INT);
        if ($pdo->execute()) {
            $pdo1 = Cnx::Connect()->prepare("DELETE FROM voiture where id=:id");
